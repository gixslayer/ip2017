...
const int xInit = 1;
const int yInit = WereldHoogte-2; //WereldHoogte/2;
...
void ROBOT :: maakBallenChaos ()	// toegevoegd door Ger; aangepast door Peter
{
    int onderste_laan = rand () % (WereldHoogte / 2) + 2 ;
    for (int laan = WereldHoogte-2; laan > onderste_laan; laan--)
    {
        int kruispunt = WereldBreedte - rand () % (WereldBreedte/2) - 3 ;
        for (int straat = WereldBreedte-2; straat > kruispunt; straat--)
            Wereld [straat][laan] = Bal ;
    }
...
void ROBOT :: maakBallenPad ()			// toegevoegd door Peter
{
	...
	for (int ix = 1; ix < WereldBreedte-4; ix++)					Wereld[ix][WereldHoogte/2]  = Bal;
	for (int iy = WereldHoogte/2; iy < WereldHoogte-4; iy++)		Wereld[WereldBreedte-5][iy] = Bal;
	for (int ix = WereldBreedte-5; ix >= WereldBreedte / 2; ix--)	Wereld[ix][WereldHoogte-5]  = Bal;
	for (int iy = WereldHoogte-5; iy >= 6; iy--)					Wereld[WereldBreedte/2][iy] = Bal;
	for (int ix = WereldBreedte / 2; ix >= 7; ix--)		    		Wereld[ix][6]               = Bal;
	for (int iy = 6; iy <= WereldHoogte / 2 - 3; iy++)				Wereld[7][iy]               = Bal;
